...
- мигрирует старый `FOREIGN_SSH_PRIVATE_KEY` из `.env` в `secrets/foreign.key`, если он уже был настроен;
...
- если существующий ключ уже пускает на `relay@FOREIGN_SSH_HOST`, пропускает admin/root bootstrap;
